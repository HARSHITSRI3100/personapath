...
const app = express();
app.set('trust proxy', 1); // Required for Render/proxied environments
connectDB();
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'OK', aiEnabled: !!process.env.GEMINI_API_KEY, ts: new Date().toISOString() });
});
...
app.listen(PORT, () => {
  ...
  console.log(`🤖 AI features: ${process.env.GEMINI_API_KEY ? 'ENABLED (Gemini)' : '❌ MISSING GEMINI_API_KEY'}`)
});
